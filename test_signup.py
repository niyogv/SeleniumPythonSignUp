...
class Test_sign_up:
    ...
    def test_createnew(self,test_invoke):
        ...
        assert check.text=='Create a new account', 'sign up page is broken'  #Checks for the validation and throws error
        ...
        assert check1.text=='Create secret recovery phrase for me', 'srp page is broken'  #Checks for the validation and throws error
        self.driver.find_element(By.XPATH, '//button').click()
        time.sleep(3)

        #cp srp page
        check2=self.driver.find_element(By.XPATH, '//*[@id="__next"]/div/div/div/div/div/a/div/article')
        assert check2.text=="I'll do this later", 'copy srp page is broken'  #Checks for the validation and throws error
        ...
        assert check3.text=='Skip saving your secret recovery phrase?', 'Skipping srp page is broken' #Checks for the validation and throws error
        ...
        assert check4.text=='Create IOMe account', 'Tos page is broken' #Checks for the validation and throws error
        self.driver.find_element(By.XPATH, '//input').click()
        self.driver.find_element(By.XPATH, '//button').click()
        time.sleep(10)
